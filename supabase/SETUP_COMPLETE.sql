-- =============================================
-- LOCALMENTE - Setup Completo Database
-- =============================================
-- Esegui questo script nella Supabase Dashboard:
...
CREATE INDEX IF NOT EXISTS idx_locations_property_type ON public.locations(property_type);
CREATE INDEX IF NOT EXISTS idx_locations_contract_type ON public.locations(contract_type);
...
CREATE INDEX IF NOT EXISTS idx_locations_price ON public.locations(price);
CREATE INDEX IF NOT EXISTS idx_locations_reference_code ON public.locations(reference_code);
...
-- Funzione per calcolare prezzo al mq
...
-- Funzione per validare coordinate
...
DROP TRIGGER IF EXISTS validate_location_coordinates ON public.locations;
...
-- Policy CLIENTS
DROP POLICY IF EXISTS "Users can view clients in their agency" ON public.clients;
...
DROP POLICY IF EXISTS "Users can create clients for their agency" ON public.clients;
...
DROP POLICY IF EXISTS "Users can update clients in their agency" ON public.clients;
...
DROP POLICY IF EXISTS "Admin and managers can delete clients" ON public.clients;
...
-- Policy LOCATIONS
DROP POLICY IF EXISTS "Users can view locations in their agency" ON public.locations;
...
DROP POLICY IF EXISTS "Users can create locations for their agency" ON public.locations;
...
DROP POLICY IF EXISTS "Users can update locations in their agency" ON public.locations;
...
DROP POLICY IF EXISTS "Admin and managers can delete locations" ON public.locations;
...
SELECT 'Setup database completato con successo! ✅' as status;
